...
assert.equal(h.version,"22.1.0");
assert.equal(h.mode,"multimodal_agricultural_product_vision_stability_os_v22_1");
assert.equal(h.vision_intelligence?.version,"22.1");
...
assert.equal(h.neural_brain?.version,"22.1");
for(const tool of ["match_visual_product","verify_visual_product_live","guard_visual_label_claim","search_visual_agronomy","get_retake_advice"]) assert.ok(h.neural_brain?.tools.includes(tool),tool);
...
assert.equal(r.version,"22.1.0");
assert.equal(r.mode,"multimodal_agricultural_product_vision_stability_os_v22_1");
assert.equal(r.runtime?.vision_intelligence?.version,"22.1");
