...
const health=await (await GET()).json();assert.equal(health.version,"29.0.0");assert.equal(health.mode,"conversational_reasoning_natural_language_os_v29");assert.equal(health.autonomous_actions?.version,"25.0");assert.equal(health.self_learning?.version,"25.0");
...
assert.equal(result.version,"29.0.0");assert.equal(result.autonomous_action?.status,"awaiting_confirmation");assert.equal(result.autonomous_action?.kind,"quotation");assert.equal(result.conversation_state.autonomous_action.id,result.autonomous_action.action_id);
