...
const health=await (await GET()).json();assert.equal(health.version,"26.0.0");assert.equal(health.mode,"github_knowledge_natural_conversation_os_v26");assert.equal(health.autonomous_actions?.version,"25.0");assert.equal(health.self_learning?.version,"25.0");
...
assert.equal(result.version,"26.0.0");assert.equal(result.autonomous_action?.status,"awaiting_confirmation");assert.equal(result.autonomous_action?.kind,"quotation");assert.equal(result.conversation_state.autonomous_action.id,result.autonomous_action.action_id);
