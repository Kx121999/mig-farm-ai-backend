...
assert.equal(h.version,"22.4.0");
assert.equal(h.mode,"multimodal_visual_recognition_pipeline_os_v22_4");
assert.equal(h.vision_intelligence?.version,"22.4");
...
assert.equal(h.human_conversation_brain?.version,"22.4");
assert.equal(h.conversion_decision_brain?.version,"22.4");
assert.equal(h.sales_employee?.version,"22.4");
assert.equal(h.sales_conversation_os?.version,"22.4");
assert.equal(h.neural_brain?.version,"22.4");
...
try{
  ...
  assert.equal(r.version,"22.4.0");
  assert.equal(r.mode,"multimodal_visual_recognition_pipeline_os_v22_4");
  assert.equal(r.runtime?.vision_intelligence?.version,"22.4");
  assert.equal(r.vision?.requires_recognition_preflight,true);
  assert.ok(["v22_4_visual_recognition_safe_fallback","neural_multimodal_visual_recognition_sales_v22_4"].includes(r.source));
  assert.doesNotMatch(String(r.reply||""),/منتج ولا شحن ولا فرع/);
} finally {if(oldKey!==undefined) process.env.OPENAI_API_KEY=oldKey;}
console.log("V22.4 API regression PASS");
