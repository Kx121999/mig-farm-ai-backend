...
try{
  ...
  assert.equal(r.mode,"multimodal_visual_recognition_pipeline_os_v22_3");
  assert.equal(r.runtime?.vision_intelligence?.version,"22.3");
  assert.equal(r.vision?.requires_recognition_preflight,true);
  assert.ok(["v22_3_visual_recognition_safe_fallback","neural_multimodal_visual_recognition_sales_v22_3"].includes(r.source));
  assert.doesNotMatch(String(r.reply||""),/منتج ولا شحن ولا فرع/);
} finally {if(oldKey!==undefined) process.env.OPENAI_API_KEY=oldKey;}
console.log("V22.3 API regression PASS");
