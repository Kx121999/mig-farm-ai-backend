...
const h=salesEmployeeHealth(); assert.ok(["16.0","17.0"].includes(h.version)); assert.ok(h.playbook_entries>=100);
process.env.OPENAI_API_KEY="test"; const nh=neuralBrainHealth(); assert.ok(["16.0","17.0"].includes(nh.version)); assert.ok(nh.tools.includes("get_business_fact")); assert.ok(nh.tools.includes("search_sales_playbook"));
