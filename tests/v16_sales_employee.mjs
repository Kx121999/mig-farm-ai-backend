...
const h=salesEmployeeHealth(); assert.equal(h.version,"16.0"); assert.ok(h.playbook_entries>=100);
process.env.OPENAI_API_KEY="test"; const nh=neuralBrainHealth(); assert.equal(nh.version,"16.0"); assert.ok(nh.tools.includes("get_business_fact")); assert.ok(nh.tools.includes("search_sales_playbook"));
