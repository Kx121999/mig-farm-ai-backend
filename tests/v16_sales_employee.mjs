...
const h=salesEmployeeHealth(); assert.ok(["16.0","17.0","18.0","19.0","20.0","21.0","22.2"].includes(h.version)); assert.ok(h.playbook_entries>=100);
process.env.OPENAI_API_KEY="test"; const nh=neuralBrainHealth(); assert.ok(["16.0","17.0","18.0","19.0","20.0","21.0","22.0","22.1","22.2"].includes(nh.version)); assert.ok(nh.tools.includes("get_business_fact")); assert.ok(nh.tools.includes("search_sales_playbook"));
