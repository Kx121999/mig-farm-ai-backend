...
const dashboard=await dashboardGet(new Request("https://example.com/api/admin-dashboard",{headers:{cookie:sessionCookie}}));assert.equal(dashboard.status,200);const body=await dashboard.json();assert.equal(body.version,"30.0.0");assert.equal(body.security.secrets_returned,false);assert.equal(JSON.stringify(body).includes("test-admin-token-28"),false);
