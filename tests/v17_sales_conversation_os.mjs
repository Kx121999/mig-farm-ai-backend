...
assert.equal(sp.version,"17.0"); assert.equal(sp.conversation_plan.friction,"trust");

const sh=salesConversationOSHealth(); assert.equal(sh.version,"17.0"); assert.ok(sh.capabilities.includes("next_best_action"));
const eh=salesEmployeeHealth(); assert.equal(eh.version,"17.0"); assert.equal(eh.conversation_os.version,"17.0");
process.env.OPENAI_API_KEY="test"; const nh=neuralBrainHealth(); assert.equal(nh.version,"17.0"); assert.equal(nh.mode,"human_sales_conversation_os");
