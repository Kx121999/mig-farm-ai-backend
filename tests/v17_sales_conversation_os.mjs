...
assert.ok(["17.0","18.0","19.0","20.0","21.0","22.2","22.5"].includes(sp.version)); assert.equal(sp.conversation_plan.friction,"trust");

const sh=salesConversationOSHealth(); assert.ok(["17.0","18.0","19.0","20.0","21.0","22.2","22.5"].includes(sh.version)); assert.ok(sh.capabilities.includes("next_best_action"));
const eh=salesEmployeeHealth(); assert.ok(["17.0","18.0","19.0","20.0","21.0","22.2","22.5"].includes(eh.version)); assert.ok(["17.0","18.0","19.0","20.0","21.0","22.2","22.5"].includes(eh.conversation_os.version));
process.env.OPENAI_API_KEY="test"; const nh=neuralBrainHealth(); assert.ok(["17.0","18.0","19.0","20.0","21.0","22.0","22.1","22.2","22.5","24.0","25.0","26.0","27.0"].includes(nh.version)); assert.ok(["human_sales_conversation_os","current_turn_semantic_human_sales_agent","conversion_decision_human_sales_agent","product_intelligence_conversion_human_sales_agent","live_product_truth_sales_action_agent","multimodal_agricultural_product_vision_sales_agent","multimodal_agricultural_product_vision_sales_agent","multimodal_visual_intent_product_precision_sales_agent","multimodal_visual_recognition_pipeline_sales_agent","semantic_multi_intent_human_conversation_agent","secure_autonomous_natural_conversation_agent","secure_github_knowledge_natural_conversation_agent","customer_brain_decision_neural_agent"].includes(nh.mode));
