...
assert.ok(["17.0","18.0"].includes(sp.version)); assert.equal(sp.conversation_plan.friction,"trust");

const sh=salesConversationOSHealth(); assert.ok(["17.0","18.0"].includes(sh.version)); assert.ok(sh.capabilities.includes("next_best_action"));
const eh=salesEmployeeHealth(); assert.ok(["17.0","18.0"].includes(eh.version)); assert.ok(["17.0","18.0"].includes(eh.conversation_os.version));
process.env.OPENAI_API_KEY="test"; const nh=neuralBrainHealth(); assert.ok(["17.0","18.0"].includes(nh.version)); assert.ok(["human_sales_conversation_os","current_turn_semantic_human_sales_agent"].includes(nh.mode));
