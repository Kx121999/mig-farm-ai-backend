...
const ui=readFileSync(new URL("../ODOO_CHAT_UI_V29_CONVERSATIONAL_REASONING_OS.txt",import.meta.url),"utf8");
for(const marker of ["UI_VERSION='29.0.0'","mig_ai_session_id_v29","function appendSafeInline","white-space:nowrap"])assert.ok(ui.includes(marker),marker);
...
assert.equal(first.version,"29.0.0");
assert.equal(first.mode,"conversational_reasoning_natural_language_os_v29");
