...
export function analyzeSalesConversation(message="",context={}){
  ...
  return {
    version:VERSION,stage:st,mood:tone,objection:obj,commercial,technical,social,
...
export function shouldUseAdaptiveSalesAgent(message="",salesTurn={}){
  ...
  // V16 intentionally lets the neural employee phrase almost every real turn; deterministic layers stay as safety fallback.
  return true;
}
...
export function salesReplyQuality(reply="",salesTurn={}){
  const text=clean(reply,8000), flags=[]; const words=text.split(/\s+/).filter(Boolean).length;
  if(/^(أهلا بك في MIG FARM|هلا بك في MIG FARM|حسب المعلومات الموجودة|بناء على طلبك)/.test(text)) flags.push("canned_opening_risk");
  if(salesTurn?.word_count<=4&&words>95&&!salesTurn?.technical) flags.push("overlong_for_short_turn");
  if((text.match(/\?/g)||[]).length>1) flags.push("too_many_questions");
  if(/تواصل معنا|واتساب/.test(text)&&!["commit","handoff"].includes(salesTurn?.stage||"")&&!/واتساب|تواصل/.test(n(salesTurn?.raw||""))) flags.push("premature_cta");
  return {version:VERSION,score:Math.max(0,100-flags.length*18),flags,stage:salesTurn?.stage||"",mood:salesTurn?.mood||""};
}

export function salesEmployeeHealth(){
  return {version:VERSION,mode:"adaptive_human_sales_employee",playbook_entries:SALES_EMPLOYEE_PLAYBOOK.length,capabilities:["free_form_conversation","adaptive_response_length","dialect_mirroring","consultative_sales","objection_handling","single_question_qualification","natural_closing","technical_to_commerce_transition","no_forced_cta","non_template_response_policy"]};
}
