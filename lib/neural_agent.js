...
export function shouldUseNeuralAgent({message="",analysis={},cognition={},plan={},salesTurn=null,image_count=0}={}){
  ...
  if(["16.0","17.0","18.0","19.0","20.0","21.0","22.2","22.3"].includes(salesTurn?.version||"")) return true;
...
,
    ...
    },
    {
      ...
      description:"Deterministic V22.3 post-recognition decision gate. Recognition must be attempted first. Then decide whether to verify a high-confidence product live, confirm a medium-confidence candidate, request alternate identity evidence, read a label panel, or proceed.",
      strict:true,
...
function instructions(locale="ar"){
  ...
  return `You are MIG FARM's V22.3 recognition-first multimodal visual product/agricultural-vision, live-product-truth and sales-action human conversation operator: a human-like consultative salesperson, senior agricultural engineer, diagnostic expert, and autonomous commerce agent. Respond in ${lang}.
...
20C) RECOGNITION BEFORE GUARD — V22.3: for every product/label image turn, inspect the pixels FIRST and call match_visual_product before asking the customer for another photo. Transcribe only text you can actually read: product name fragments, brand, SKU, barcode digits and category clues. Empty/unclear fields are allowed; guessed fields are forbidden.
...
export async function runNeuralAgent({message="",locale="ar",context={},toolHandlers={},images=[],allowedTools=null}={}){
  ...
  const preflightText=forceProductRecognition?`\n\nV22.3 PRODUCT RECOGNITION PREFLIGHT (mandatory): inspect the attached image before any retake/clarification. Your FIRST tool call must be match_visual_product. Pass only actually readable product-name/brand/SKU/barcode/text fragments; leave unreadable fields empty. Do not call get_retake_advice or plan_visual_product_action until match_visual_product has returned.`:"";
...
export function neuralBrainHealth(){
  return {
    version:"22.3",mode:"multimodal_visual_recognition_pipeline_sales_agent",knowledge_layer:"22.3",configured:configured(),provider:configured()?"openai":"deterministic_v10_fallback",
