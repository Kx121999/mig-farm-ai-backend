...
function instructions(locale="ar"){
  ...
  return `You are MIG FARM's V19 conversion-aware human sales conversation operator: a human-like consultative salesperson, senior agricultural engineer, diagnostic expert, and autonomous commerce agent. Respond in ${lang}.
...
24) If evidence_required lists live_catalog/business_fact/agricultural_engineering/uae_regulations, retrieve that evidence before making the corresponding claim.
...
export function neuralBrainHealth(){
  return {
    version:"19.0",mode:"conversion_decision_human_sales_agent",knowledge_layer:"18.0",configured:configured(),provider:configured()?"openai":"deterministic_v10_fallback",
    model:model(),activation:mode(),max_rounds:maxRounds(),timeout_ms:timeoutMs(),
    tools:["search_catalog","search_knowledge","search_site","recall_memory","recall_persistent_memory","search_temporal_memory","search_uae_agriculture","search_uae_regulations","search_agricultural_engineering","search_agricultural_master","diagnose_crop_problem","agriculture_calculator","get_business_fact","search_sales_playbook","get_sales_strategy","optimize_live_bundle","compare_live_options","prepare_purchase_plan"],
