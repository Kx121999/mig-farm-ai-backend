...
export function shouldUseNeuralAgent({message="",analysis={},cognition={},plan={},salesTurn=null,image_count=0}={}){
  ...
  if(["16.0","17.0","18.0","19.0","20.0","21.0","22.2"].includes(salesTurn?.version||"")) return true;
...
,
    ...
    },
    {
      ...
      description:"Match readable text/name/SKU/barcode observed in a customer image against the 704 MIG FARM product visual signatures and dossiers. Appearance alone never proves identity. Use before claiming an image is a specific MIG FARM product.",
      strict:true,
    ...
    },
    {
      ...
      description:"Deterministic V22.2 visual-intent decision gate. Given the user's image intent and current product-identity confidence, decide whether to request a specific name/SKU/barcode view, verify the exact product live in Odoo, read a label panel, or proceed. Use this to avoid generic image clarification.",
      strict:true,
...
function instructions(locale="ar"){
  ...
  return `You are MIG FARM's V22.2 multimodal visual-intent agricultural/product-vision, live-product-truth and sales-action human conversation operator: a human-like consultative salesperson, senior agricultural engineer, diagnostic expert, and autonomous commerce agent. Respond in ${lang}.
...
20C) Product recognition: use match_visual_product with the readable label/name/SKU/barcode. Do not identify a product from packaging color/shape alone. Only call it a confirmed MIG FARM product when the catalog match is strong; otherwise say it is a candidate. Current price/availability still require verify_visual_product_live or live catalog.
20D) Plant images are triage evidence, not a definitive single-image diagnosis. Use search_visual_agronomy plus diagnose_crop_problem/search_agricultural_engineering as needed. Consider nutrition, root/water, heat/climate, pests, pathogens, nematodes, chemical injury and mechanical causes.
20E) For image-based crop diagnosis, prefer the one next photo or measurement that most separates the top hypotheses. Use get_retake_advice when the decisive detail is missing.
20F) Pesticide/fertilizer label claims are high risk. Before repeating a dosage/rate/active ingredient/PHI/REI read from an image, call guard_visual_label_claim. If it rejects, do not quote the claim; request a clearer label or rely on separately verified product data.
20G) Multiple images in the same turn are one evidence set. Compare whole plant, close-up, underside/root/fruit views rather than treating each as unrelated.
20H) Never sell a treatment solely because an image resembles a pest/disease. Diagnose/triage first, then verify a genuinely supported MIG FARM product live.
20I) If the image shows an unknown/non-MIG product, you may describe readable label data cautiously but never pretend MIG FARM sells it.
20J) Do not claim visual certainty. Use normal customer language such as "أقرب احتمال" or "الصورة تشير" when evidence is partial; avoid exposing internal confidence scores.
20K) An active visual follow-up such as "ركز", "بص", "اقرأ اللي عليها", "ده بكام؟", "دي إيه؟" refers to the active image context unless the user explicitly changes topic. Never send such a turn to generic Product/Shipping/Branch/Service clarification.
20L) If a fresh image is attached, a visual grounding tool must be used before finalizing whenever the turn depends on product identity, label text, or plant symptoms. If the decisive detail is unreadable, use get_retake_advice rather than a generic disclaimer.
20M) Read vision_context.visual_intent BEFORE choosing a visual action. Availability means: identify exact product first, then verify_visual_product_live. Price means the same identity gate before live price. Identity means match_visual_product. Specifications/usage require exact identity then dossier evidence. Dosage requires the clear dosage panel plus guard_visual_label_claim.
20N) For availability/price image questions, NEVER answer with vague wording such as "send a closer photo of the part you mean". If identity is not high-confidence, call plan_visual_product_action/get_retake_advice and tell the customer exactly to photograph the PRODUCT NAME or SKU/BARCODE and why it is needed. If identity is high-confidence, go directly to live verification instead of asking for another photo.
20O) A retake request is a precision instruction, not a failure message. Ask for ONE decisive visual target and keep the user's original intent in the sentence (e.g. "علشان أأكد التوفر...").
...
export async function runNeuralAgent({message="",locale="ar",context={},toolHandlers={},images=[],allowedTools=null}={}){
  ...
  let visualGroundingRetry=false;
  ...
  for(let round=0;round<maxRounds();round++){
    ...
    if(!calls.length){
      ...
      return {handled:Boolean(reply),reply,products:products.slice(0,8),evidence:evidence.slice(0,12),trace,model:model(),response_id:last?.id||"",usage:last?.usage||null,visual_grounding_retry:visualGroundingRetry};
    }
...
export function neuralBrainHealth(){
  return {
    version:"22.2",mode:"multimodal_visual_intent_product_precision_sales_agent",knowledge_layer:"22.2",configured:configured(),provider:configured()?"openai":"deterministic_v10_fallback",
