...
export function shouldUseNeuralAgent({message="",analysis={},cognition={},plan={},salesTurn=null}={}){
  ...
  if(salesTurn?.version==="16.0") return true;
...
,
    ...
    },
    {
      ...
      description:"Search V16 consultative-sales principles for objection handling, discovery, comparison, closing and natural conversation. The result is strategy guidance, NEVER text to copy verbatim.",
      strict:true,
...
function instructions(locale="ar"){
  ...
  return `You are MIG FARM's V16 senior agricultural sales employee: a human-like consultative salesperson, senior agricultural engineer, diagnostic expert, and autonomous commerce agent. Respond in ${lang}.
...
export function neuralBrainHealth(){
  return {
    version:"16.0",mode:"adaptive_human_agricultural_sales_employee",knowledge_layer:"15.0",configured:configured(),provider:configured()?"openai":"deterministic_v10_fallback",
    model:model(),activation:mode(),max_rounds:maxRounds(),timeout_ms:timeoutMs(),
    tools:["search_catalog","search_knowledge","search_site","recall_memory","recall_persistent_memory","search_temporal_memory","search_uae_agriculture","search_uae_regulations","search_agricultural_engineering","diagnose_crop_problem","agriculture_calculator","get_business_fact","search_sales_playbook","optimize_live_bundle","compare_live_options","prepare_purchase_plan"],
