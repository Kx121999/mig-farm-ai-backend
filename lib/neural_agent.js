...
export function shouldUseNeuralAgent({message="",analysis={},cognition={},plan={},salesTurn=null}={}){
  ...
  if(["16.0","17.0","18.0"].includes(salesTurn?.version||"")) return true;
...
function instructions(locale="ar"){
  ...
  return `You are MIG FARM's V18 current-turn-first human sales conversation operator: a human-like consultative salesperson, senior agricultural engineer, diagnostic expert, and autonomous commerce agent. Respond in ${lang}.

CURRENT TURN SEMANTIC PRIORITY — V18:
...
V18 HUMAN CONVERSATION BRAIN + SALES CONVERSATION OS:
...
export function neuralBrainHealth(){
  return {
    version:"18.0",mode:"current_turn_semantic_human_sales_agent",knowledge_layer:"18.0",configured:configured(),provider:configured()?"openai":"deterministic_v10_fallback",
    model:model(),activation:mode(),max_rounds:maxRounds(),timeout_ms:timeoutMs(),
    tools:["search_catalog","search_knowledge","search_site","recall_memory","recall_persistent_memory","search_temporal_memory","search_uae_agriculture","search_uae_regulations","search_agricultural_engineering","search_agricultural_master","diagnose_crop_problem","agriculture_calculator","get_business_fact","search_sales_playbook","optimize_live_bundle","compare_live_options","prepare_purchase_plan"],
