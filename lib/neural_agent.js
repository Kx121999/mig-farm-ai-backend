...
export function shouldUseNeuralAgent({message="",analysis={},cognition={},plan={},salesTurn=null}={}){
  ...
  if(["16.0","17.0"].includes(salesTurn?.version||"")) return true;
...
function instructions(locale="ar"){
  ...
  return `You are MIG FARM's V17 human sales conversation operator: a human-like consultative salesperson, senior agricultural engineer, diagnostic expert, and autonomous commerce agent. Respond in ${lang}.
...
V17 HUMAN SALES CONVERSATION OS:
...
export function neuralBrainHealth(){
  return {
    version:"17.0",mode:"human_sales_conversation_os",knowledge_layer:"15.0",configured:configured(),provider:configured()?"openai":"deterministic_v10_fallback",
