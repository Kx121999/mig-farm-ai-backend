...
const DEFAULT_REMOTE_BASE="https://raw.githubusercontent.com/Kx121999/mig-farm-ai-backend/main/knowledge_V26";
const MAX_REMOTE_PACK_BYTES=25*1024*1024;
let manifestCache=null,routerCache=null;
const packCache=new Map();
const packPromiseCache=new Map();
...
function remoteBase(){
  const value=clean(process.env.MIG_V26_KNOWLEDGE_BASE_URL||DEFAULT_REMOTE_BASE,2000).replace(/\/+$/,"");
  const parsed=new URL(`${value}/`);
  if(parsed.protocol!=="https:")throw new Error("knowledge_base_url_must_use_https");
  return parsed.toString().replace(/\/+$/,"");
}
function remoteTimeoutMs(){return Math.max(4000,Math.min(20000,Number(process.env.MIG_V26_REMOTE_TIMEOUT_MS)||12000));}
...
async function fetchPackBytes(file,meta){
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),remoteTimeoutMs());
  try{
    const url=`${remoteBase()}/packs/${encodeURIComponent(file)}`;
    const response=await fetch(url,{signal:controller.signal,headers:{Accept:"application/jsonl,text/plain;q=0.9,*/*;q=0.1","User-Agent":"MIG-FARM-AI-V26"}});
    if(!response.ok)throw new Error(`knowledge_pack_http_${response.status}`);
    const declared=Number(response.headers.get("content-length")||0);
    if(declared>MAX_REMOTE_PACK_BYTES)throw new Error("knowledge_pack_too_large");
    ...
    if(meta.sha256){
      ...
      if(actual!==String(meta.sha256).toLowerCase())throw new Error("knowledge_pack_integrity_mismatch");
    }
    return bytes;
...
async function loadPack(file){
  ...
  if(packPromiseCache.has(file))return packPromiseCache.get(file);
  const meta=packMeta(file);if(!meta)return [];
  const pending=(async()=>{
    const bytes=await fetchPackBytes(file,meta);
    const rows=[];
    for(const line of bytes.toString("utf8").split("\n")){
      if(!line)continue;
      try{rows.push(JSON.parse(line));}catch{throw new Error("knowledge_pack_invalid_jsonl");}
    }
    packCache.clear();packCache.set(file,rows);return rows;
  })();
  packPromiseCache.set(file,pending);
  try{return await pending;}finally{packPromiseCache.delete(file);}
}
...
export async function searchConversationKnowledgeV26(query="",{limit=6,domain=""}={}){
  const text=n(query);if(!text||!conversationKnowledgeHealth().ready)return {items:[],packs_scanned:[],reason:"not_ready"};
  const queryTokens=uniq(tokenize(text).filter(x=>x.length>=2));const intent=queryIntent(text);const files=selectPacks(text,domain);const hits=[];const errors=[];
  const loaded=await Promise.allSettled(files.map(file=>loadPack(file)));
  for(let index=0;index<loaded.length;index++){
    const result=loaded[index],file=files[index];
    if(result.status==="rejected"){errors.push({file,code:clean(result.reason?.message||"knowledge_pack_unavailable",120)});continue;}
  ...
  return {items:unique,packs_scanned:files,intent,engine:"v26_remote_sharded_jsonl_retrieval",reason:errors.length&&!unique.length?"remote_pack_unavailable":undefined,errors:errors.length?errors:undefined};
}

export function conversationKnowledgeHealth(){
  const m=manifest();const total=Number(m.total_pack_bytes||0);
  return {version:VERSION,mode:"remote_sharded_conversation_knowledge",ready:Boolean(m.version===VERSION&&arr(m.packs).length&&total),packs:arr(m.packs).length,records:Number(m.total_records||0),bytes:total,megabytes:Number((total/1048576).toFixed(2)),target_megabytes:Number(m.target_megabytes||400),max_pack_megabytes:Number(m.max_pack_megabytes||0),browser_upload_safe:Boolean(m.github?.browser_upload_safe),retrieval:"local_manifest_router+remote_integrity_verified_pack+bounded_pack_scan",storage:"remote_object_packs",function_bundle:"manifest_router_only",cache:"one_pack_lru+inflight_deduplication",price_stock_policy:"live_odoo_only",dosage_policy:"official_label_or_verified_product_data_only"};
}
