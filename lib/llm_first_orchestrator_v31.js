...
function model(){return clean(process.env.OPENAI_INTENT_MODEL||process.env.OPENAI_MODEL||"gpt-5.6",100);}
...
function deterministicFallback({message="",legacyAnalysis={},state={}}={}){
  const text=normalizeAr(clean(message,2000)),trimmed=text.replace(/[؟?!.,،]/g,"").trim();let intent=normalizeIntent(legacyAnalysis?.intent),domain="unclear",safe=null,relationship="new_topic",guarded=false;
  ...
  else if(/(?:جرع|مل لكل|سم3|معدل استخدام)/.test(trimmed)){intent="dosage";domain="products";}
  ...
  const confidence=guarded ? .96 : .55;
  return sanitizeFrame({language:"ar",dialect:"unknown",domain,primary_intent:intent,intents:[{name:intent,confidence}],speech_act:"question",topic_relationship:relationship,context_policy:{use_recent_context:relationship!=="new_topic",ignore_old_product:relationship==="new_topic"&&!/سعر|متوفر|منتج|استخدام|جرع/.test(trimmed),ignore_old_agriculture:relationship==="new_topic"&&(domain==="social"||domain==="mig_farm_business"),requires_visible_choice:false},entities:{emirate:null,crop:null,cultivation:null,category:null,product_name:null,product_reference:null,quantity:null,budget_aed:null,symptoms:[],decision_criteria:[]},reference:{requires_context:false,target:"none",resolved_text:null,confidence:0},ambiguity:{required:false,missing_information:null,question:null},response_plan:{mode:safe?"social":BUSINESS.has(intent)?"tool_grounded":"natural_direct",external_facts_required:BUSINESS.has(intent)||![...SOCIAL].includes(intent),answer_order:[intent],max_questions:0,tone:"natural"},compound:false,safe_direct_reply:safe,meaning_summary:intent,confidence},guarded?"deterministic_guard":"deterministic_fallback");
}
...
- Return only the requested structured JSON. Do not provide reasoning or chain-of-thought.`;
  ...
  try{
    ...
    const frame=sanitizeFrame(raw,"openai");stats.openai+=1;if(frame.topic_relationship==="new_topic")stats.new_topics+=1;else stats.followups+=1;
    cache.set(key,{at:Date.now(),frame});while(cache.size>200)cache.delete(cache.keys().next().value);
    return frame;
  }catch(error){stats.failures+=1;stats.fallbacks+=1;return {...deterministicFallback({message,legacyAnalysis,state}),provider_error:clean(error?.name==="AbortError"?"timeout":error?.message,180)};}
...
export function applyMeaningFrameV31(analysis={},semanticFrame={},frame={}){
  ...
  const crop=cropKey(frame.entities?.crop);if(crop&&CROPS[crop])analysis.crop={key:crop,labelAr:CROPS[crop].labelAr,aliases:CROPS[crop].aliases||[]};
...
export function meaningFrameClientV31(frame={}){return {version:VERSION,provider:frame.provider,authoritative:Boolean(frame.authoritative),domain:frame.domain,primary_intent:frame.primary_intent,intents:frame.intents,speech_act:frame.speech_act,topic_relationship:frame.topic_relationship,context_policy:frame.context_policy,reference:frame.reference,ambiguity:frame.ambiguity,response_plan:frame.response_plan,meaning_summary:frame.meaning_summary,confidence:frame.confidence};}
