...
const LOCAL_SEMANTIC_CONCEPTS={
  ...
  identity:[/(?:مين|من|ايه|ما|شو|وش)\s+(?:انت|أنت|دورك|وظيفتك|صاحب|وراء|ورا|مسؤول)/,/(?:عرف|تعرف|قدم)\p{L}*\s+(?:بنفسك|نفسك|عنك|عليك)/u,/(?:برنامج|بوت|روبوت|ذكاء|موظف|انسان|مساعد\s+(?:الي|آلي|ذكي)|المجيب|نظام\s+ذكي)(?:\s+(?:ولا|او|أو)|\s*$)/,/(?:اسمك|هويتك|دورك|وظيفتك)|(?:وظيفه|وظيفة|دور|مهمه|مهمة).{0,16}(?:المساعد|البوت|النظام|الذكاء)/,/(?:مين|من)\s+(?:اللي\s+)?\p{L}*(?:جاوب|رد)\p{L}*|(?:الرد|الاجابه|الاجابات)\s+.{0,16}(?:انسان|موظف|برنامج|بوت|مسؤول)/u,/(?:انت|أنت).{0,18}(?:بتعمل|تعمل|مهمتك).{0,10}(?:ايه|إيه|ماذا)|(?:انت|أنت)\s+(?:عباره|عبارة)\s+عن/],
...
function deterministicFallback({message="",legacyAnalysis={},legacySemanticFrame={},state={}}={}){
  ...
  else if(localSemantic.intent==="correction"){
  ...
  else if(localSemantic.intent==="help_request"){
  ...
  else if(localSemantic.intent==="identity"){
  ...
  else if(localSemantic.intent==="price"){
  ...
  else if(localSemantic.intent==="suitability"){
  ...
  else if(/^(?:اسمك ايه|اسمك شو|شو اسمك|ما اسمك|مين انت|من انت|انت مين|who are you|what is your name)$/.test(trimmed)){replaceIntent("identity");domain="social";safe="أنا MIG FARM AI، مساعدك للمنتجات والزراعة وخدمات MIG FARM. قول لي محتاج إيه وأنا أساعدك.";guarded=true;}
  ...
  else if(/(?:اسمك|مين انت|من انت)/.test(trimmed)){replaceIntent("identity");domain="social";safe="أنا MIG FARM AI، مساعدك للمنتجات والزراعة وخدمات MIG FARM.";guarded=true;}
  ...
  if(!followup&&expected&&short&&!SOCIAL.has(intent)&&!BUSINESS.has(intent))relationship="answer_to_assistant";
  const confidence=guarded ? Math.max(intent==="diagnosis"?.90:.88,localSemantic.intent===intent?localSemantic.confidence:0) : .55;
  if(!intentNames.includes(intent))intentNames.unshift(intent);
  const answerOrder=intentNames.filter(name=>name!=="unknown");
