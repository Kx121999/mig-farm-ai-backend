...
function deterministicFallback({message="",legacyAnalysis={},legacySemanticFrame={},state={}}={}){
  ...
  return sanitizeFrame({language:"ar",dialect:"unknown",domain,primary_intent:intent,intents:intentNames.map(name=>({name,confidence})),speech_act:symptoms.length?"statement":speechAct,topic_relationship:relationship,context_policy:{use_recent_context:relationship!=="new_topic",ignore_old_product:relationship==="new_topic"&&!/سعر|متوفر|منتج|استخدام|جرع/.test(trimmed),ignore_old_agriculture:relationship==="new_topic"&&(domain==="social"||domain==="mig_farm_business"),requires_visible_choice:false},entities:{emirate:null,crop,cultivation:null,category:null,product_name:null,product_reference:null,quantity:null,budget_aed:null,symptoms,decision_criteria:attributes.criteria},reference:{requires_context:followup,target:followup?"previous_question":"none",resolved_text:followup?clean(stored?.last_question,300)||null:null,confidence:followup?0.96:0},ambiguity,response_plan:{mode:safe?"social":answerOrder.some(name=>BUSINESS.has(name))?"tool_grounded":responseMode,external_facts_required:answerOrder.some(name=>BUSINESS.has(name))||!answerOrder.every(name=>SOCIAL.has(name)),answer_order:answerOrder,max_questions:maxQuestions,tone:"natural"},compound:answerOrder.length>1,safe_direct_reply:safe,meaning_summary:summary||answerOrder.join(" + ")||intent,confidence},guarded?"deterministic_guard":"deterministic_fallback");
}
