...
function model(){ return normalizeProviderModelV41(process.env.OPENAI_INTENT_MODEL||process.env.OPENAI_MODEL||"gpt-5-mini"); }
...
- Return only the requested structured JSON. Do not provide reasoning or chain-of-thought.`;
  ...
  try{
    const data=await callOpenAIResponsesV41({model:model(),store:false,instructions,input:[{role:"user",content:[{type:"input_text",text:JSON.stringify(input)}]}],text:{format:{type:"json_schema",name:"mig_farm_meaning_v31",strict:true,schema:schema()}}},{purpose:"meaning_interpretation",timeout_ms:timeoutMs(),max_attempts:2});
    ...
    const detail=clean(error?.code||error?.message||"provider_failure",180);
    stats.last_error=detail||"provider_failure";
