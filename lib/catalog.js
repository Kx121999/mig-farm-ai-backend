...
import { rankCatalogProducts } from "./ranker.js";
...
function cropMatches(p,cropKey){
  ...
  const hay=productHay(p);
  const namedMarkers=MIG_SEED_CATALOG
    .filter(x=>x.crop===cropKey)
    .flatMap(x=>[x.nameAr,...(x.aliases||[])])
    .map(n)
    .filter(Boolean);

  // If MIG FARM has a curated variety list for this crop, it is the source of truth.
  // Do NOT fall back to a generic crop word found in SEO/global page text; that was
  // allowing cucumber/eggplant cards to leak into a tomato search.
  if(namedMarkers.length) return namedMarkers.some(m=>hay.includes(m));

  // Only crops without a curated MIG list use the weaker crop-name + MIG brand fallback.
...
export function filterRankProducts(products=[],analysis,state,message=""){
  ...
  // V8 intelligent ranking:
  // exact query > crop/category fit > MIG FARM brand > availability > budget.
  return rankCatalogProducts(list,analysis,state,message,8);
}
