...
export function extractOdooProductIds(html=""){
  const source=String(html||"");
  const patterns={
    product_template_id:[
      /data-product-template-id=["'](\d+)["']/i,
      /name=["']product_template_id["'][^>]*value=["'](\d+)["']/i,
      /value=["'](\d+)["'][^>]*name=["']product_template_id["']/i,
      /["']product_template_id["']\s*:\s*(\d+)/i,
      /["']product_tmpl_id["']\s*:\s*(\d+)/i
    ],
    product_id:[
      /data-product-product-id=["'](\d+)["']/i,
      /data-product-id=["'](\d+)["']/i,
  ...
  const out={product_template_id:null,product_id:null};
  for(const [key,list] of Object.entries(patterns)){
    for(const pattern of list){
      const m=source.match(pattern);
      if(m&&Number(m[1])>0){out[key]=Number(m[1]);break;}
    }
  }
  return out;
}

function productImage(ld={},html=""){
  const image=ld?.image;
  if(typeof image==="string"&&image) return absoluteUrl(image)||image;
  if(Array.isArray(image)&&image[0]){
    const value=typeof image[0]==="string"?image[0]:image[0]?.url;
    if(value) return absoluteUrl(value)||String(value);
  }
  if(image&&typeof image==="object"&&image.url) return absoluteUrl(image.url)||String(image.url);
  const og=metaContent(html,"og:image");
  return og?(absoluteUrl(og)||og):"";
}
...
export async function fetchProduct(url,{force=false}={}){
  const cached=globalCache.products.get(url);
  const now=Date.now();
  if(!force && cached && now-cached.time<CACHE_TTL) return cached.value;
  ...
  const odooIds=extractOdooProductIds(html);
  const product={
    ...
    image:productImage(ld,html),
    product_template_id:odooIds.product_template_id,
    product_id:odooIds.product_id,
    url,
    truth:{
      source:"odoo_live_product_page",
