...
function config(){
  return {enabled:enabled(),apiKey:clean(process.env.OPENAI_API_KEY||"",5000),model:normalizeProviderModelV41(process.env.OPENAI_MODEL||"gpt-5-mini"),vectorStoreId:clean(process.env.OPENAI_VECTOR_STORE_ID||"",300),timeout:Math.max(1500,Math.min(15000,Number(process.env.MIG_ENTERPRISE_RETRIEVAL_TIMEOUT_MS)||6000))};
}
...
async function externalFileSearch(query,limit){
  ...
  try{
    const data=await callOpenAIResponsesV41({
    ...
    },{purpose:"enterprise_retrieval",timeout_ms:cfg.timeout,max_attempts:2});
  ...
  }catch(error){return {items:[],reason:clean(error?.code||error?.message||"external_error",160)};}
}
