...
export function naturalIntentReply(message,locale="ar"){
  const t=normalizeAr(message);

  if(locale==="en"){
    if(/other than seeds|besides seeds|apart from seeds|what else do you have|what do you sell/.test(t)){
      return "Sure. Besides seeds, MIG FARM also offers fertilizers and plant nutrition, pesticides, irrigation and hydroponics supplies, tools and equipment, greenhouse solutions, and agricultural services. Tell me which section you want and I’ll help you with it.";
    }
    return "";
  }

  const asksBeyondSeeds =
    /(غير|بدون|ما عدا|بعيد عن).{0,18}(البذور|بذور)/.test(t) ||
    /(البذور|بذور).{0,18}(غيرها|غير|سواها|سوا)/.test(t);

  if(asksBeyondSeeds){
    return "هيه أكيد 🌱 غير البذور عندنا أسمدة وتغذية النبات، مبيدات، أنظمة الري والزراعة المائية، أدوات ومعدات زراعية، حلول وتجهيز البيوت المحمية، وخدمات واستشارات زراعية. إذا تبا، قل لي أي قسم منهم وأنا أطلع لك التفاصيل الموجودة في الموقع.";
  }

  const asksWhatWeHave =
    /(شو عندكم|وش عندكم|ايش عندكم|إيش عندكم|ايه عندكم|إيه عندكم|شو تبيعون|وش تبيعون|اقسامكم|أقسامكم|منتجاتكم)/.test(t);

  if(asksWhatWeHave){
    return "حياك 🌱 عند MIG FARM نغطي احتياجات الزراعة من البذور والأسمدة وتغذية النبات والمبيدات والري والزراعة المائية والأدوات والمعدات والبيوت المحمية والخدمات الزراعية. خبرني شو اللي تدور عليه وأنا أساعدك فيه.";
  }
...
export function smallTalk(message,locale="ar"){
  ...
  if(/^(هلا(?: والله)?|مرحبا|السلام عليكم|سلام|هاي|hello|hi)[\s.!؟]*$/.test(t)){
