...
export function analyzeAgriculturalRequest(message="",ctx={}){
  ...
  const crop=detectedCrop(normalized)||ctx?.analysis?.crop?.key||ctx?.state?.crop||"";
  const system=detectSystem(normalized)||ctx?.analysis?.cultivation||ctx?.state?.cultivation||"";
  const stage=detectStage(normalized); const zone=detectZone(normalized); const intent=detectIntent(normalized);
...
export function diagnoseAgriculturalProblem(message="",context={}){
  ...
  return {
    handled:Boolean(matched.length||fallback.length),frame,
