...
const INTENT_PRIORITY=["correction","order_status","purchase","diagnosis","dosage","comparison","recommendation","suitability","product_search","product_details","price","availability","shipping","delivery_time","branches","hours","payment","returns","contact","social","greeting","general_question"];
...
function buildPlan(intents,reference,entities,{dialect,canonical}){
  ...
  const multi=ordered.filter(x=>!["correction","social","greeting","general_question"].includes(x)).length>1;
  let route="general_conversation";
  if((names.includes("greeting")||names.includes("social"))&&names.length===1)route="social";
...
const LEGACY_INTENTS={greeting:"greeting",order_status:"order_status",purchase:"purchase",diagnosis:"plant_problem",recommendation:"recommendation",product_search:"product_search",shipping:"shipping",delivery_time:"delivery_time",branches:"branches",hours:"hours",payment:"payment",returns:"returns",contact:"contact"};
...
export function enrichAnalysisWithSemanticFrame(analysis={},frame={}){
  ...
  if(["unknown","agriculture_general"].includes(String(out.intent||""))){
    ...
    const selected=["order_status","purchase","diagnosis","recommendation","product_search","delivery_time","shipping","branches","hours","payment","returns","contact","greeting"].find(name=>semanticNames.includes(name));
...
export function semanticHumanBrainHealth(){
  return {version:VERSION,mode:"semantic_human_conversation_orchestrator_os_v24",capabilities:["egyptian_dialect","emirati_dialect","gulf_dialect","levantine_dialect","msa","english","arabizi_normalization","code_switch_detection","multi_intent_decomposition","pronoun_resolution","ordinal_product_resolution","correction_supersession","current_turn_isolation","one_question_clarification","evidence_task_planning","unified_tool_budget","answer_order_contract","live_truth_routing","natural_dialect_mirroring"]};
}
