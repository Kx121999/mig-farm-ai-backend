...
const ARABIZI_WORDS=new Map([
  ...
  ["feen","فين"],["fen","فين"],["ween","وين"],["wen","وين"],["makan","مكان"],["mkan","مكان"],
  ["3nwan","عنوان"],["enwan","عنوان"],["branch","فرع"],["for3","فرع"],
  ["mkanokom","مكانكم"],["mkanokm","مكانكم"],["mkankom","مكانكم"],["mkankm","مكانكم"]
]);
...
const INTENT_DEFS=[
  ...
  ["branches",/(?:مكانكم\s*(?:فين|وين|اين)|(?:فين|وين|اين)\s*مكانكم|موقعكم\s*(?:فين|وين|اين)|(?:فين|وين|اين)\s*موقعكم|عنوانكم|وينكم|فينكم|موقعكم|فروع|فرعكم|اقرب فرع|where are you|where is your (?:shop|store)|location|branches?)/],
  ...
  ["pickup",/(?:استلام من الفرع|استلام شخصي|pickup|click and collect)/],
  ["tax",/(?:ضريبه|ضريبة|فاتوره ضريبيه|فاتورة ضريبية|vat|tax)/],
  ["privacy",/(?:سياسه الخصوصيه|سياسة الخصوصية|privacy policy|بياناتي)/],
  ["terms",/(?:الشروط والاحكام|الشروط والأحكام|terms and conditions)/],
  ["cookies",/(?:سياسه الكوكيز|سياسة الكوكيز|ملفات الارتباط|cookies?)/],
  ["company",/(?:من نحن|عن الشركه|عن الشركة|عن ميج فارم|عن mig farm|about mig farm)/],
  ["categories",/(?:بتبيعوا ايه|بتبيعوا إيه|شو تبيعون|وش تبيعون|عندكم ايه|عندكم إيه|شو عندكم|وش عندكم|اقسامكم|أقسامكم|what do you sell)/],
  ["services",/(?:خدماتكم|ايه الخدمات|إيه الخدمات|شو الخدمات|وش الخدمات|what services)/],
...
const TASKS={
  ...
  pickup:{tool:"get_business_fact",evidence:"pickup_policy"},tax:{tool:"get_business_fact",evidence:"tax_policy"},company:{tool:"get_business_fact",evidence:"company_fact"},services:{tool:"get_business_fact",evidence:"service_fact"},
...
const INTENT_PRIORITY=["correction","identity","order_status","purchase","diagnosis","dosage","comparison","recommendation","suitability","product_search","product_details","price","availability","shipping","delivery_time","branches","hours","payment","returns","contact","pickup","tax","privacy","terms","cookies","company","categories","services","social","greeting","general_question"];
...
function buildPlan(intents,reference,entities,{dialect,canonical}){
  ...
  else if(names.some(x=>["shipping","delivery_time","branches","hours","payment","returns","contact","order_status","pickup","tax","privacy","terms","cookies","company","categories","services"].includes(x)))route="business_fact";
...
export function buildSemanticFrame({message="",analysis={},state={},history=[],selectedProduct=null,selectedProducts=[]}={}){
  ...
  const contextScope=repair.supersede_old_topic||plan.route==="social"||plan.route==="business_fact"?"current_turn_only":reference.depends_on_context?"explicit_reference":"current_topic_plus_relevant_history";
...
export function enrichAnalysisWithSemanticFrame(analysis={},frame={}){
  ...
  if(frame.plan?.route==="business_fact"&&!frame.compound?.is_multi_intent){
    const semanticNames=frame.intents?.map(x=>x.name)||[];
    const selected=["order_status","delivery_time","shipping","branches","hours","payment","returns","contact","pickup","tax","privacy","terms","cookies","company","categories","services"].find(name=>semanticNames.includes(name));
    if(selected)out.intent=LEGACY_INTENTS[selected]||out.intent;
  }else if(["unknown","agriculture_general"].includes(String(out.intent||""))){
    const semanticNames=frame.intents?.map(x=>x.name)||[];
...
export function semanticHumanBrainHealth(){
  return {version:VERSION,mode:"natural_semantic_customer_brain_orchestrator_os_v27",capabilities:["customer_brain_interop","current_turn_sovereignty","business_fact_isolation","egyptian_dialect","emirati_dialect","gulf_dialect","levantine_dialect","msa","english","arabizi_normalization","code_switch_detection","identity_and_social_isolation","multi_intent_decomposition","pronoun_resolution","ordinal_product_resolution","correction_supersession","current_turn_isolation","one_question_clarification","evidence_task_planning","unified_tool_budget","answer_order_contract","live_truth_routing","natural_dialect_mirroring"]};
}
