...
export function enrichLiveProductsWithDossiers(products=[],{descriptionChars=1100}={}){
  ...
  return arr(products).map(item=>{
    ...
    if(!p)return item;
    return {...item,product_dossier:{external_id:p.external_id,sku:p.sku,category:p?.taxonomy?.category||"",supplier:arr(p?.taxonomy?.supplier),type:arr(p?.taxonomy?.type),feature:arr(p?.taxonomy?.feature),description_provenance:p?.descriptions?.provenance||"",description:clean(p?.descriptions?.sales_exact||p?.descriptions?.ecommerce_text_exact||"",descriptionChars)}};
  });
}
