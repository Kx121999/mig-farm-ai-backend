...
function levDistance(a='',b=''){
  const A=Array.from(n(a)),B=Array.from(n(b));if(!A.length)return B.length;if(!B.length)return A.length;
...
function tokenCanon(v=''){let x=n(v);if(x.startsWith('ال')&&x.length>4)x=x.slice(2);return x;}
function arabicSkeleton(v=''){const x=tokenCanon(v);const s=x.replace(/[اوي]/g,'');return s.length>=3?s:x;}
function tokenSimilarity(a='',b=''){
  const A=tokenCanon(a),B=tokenCanon(b);if(!A||!B)return 0;if(A===B)return 1;if(A.includes(B)||B.includes(A))return Math.min(A.length,B.length)/Math.max(A.length,B.length);
  const SA=arabicSkeleton(A),SB=arabicSkeleton(B);if(SA===SB&&SA.length>=3)return .9;
  const d=levDistance(A,B);return 1-d/Math.max(A.length,B.length);
}
export function resolveProductEntityFuzzy(query='',{limit=6,minTokenSimilarity=.68,descriptionChars=1800}={}){
  buildIndexes();const qt=clean(query,900),qTokens=toks(qt).map(tokenCanon).filter(x=>x.length>2);if(!qTokens.length)return [];
  const rows=[];
  for(const row of PRODUCT_INDEX){
    const p=loadDossiers().products[row.i];const titleTokens=toks(`${p.name||''} ${p.sku||''} ${arr(p?.retrieval?.aliases).join(' ')}`).map(tokenCanon).filter(Boolean);
    if(!titleTokens.length)continue;let sum=0,minBest=1;
    for(const q of qTokens){let best=0;for(const t of titleTokens)best=Math.max(best,tokenSimilarity(q,t));sum+=best;minBest=Math.min(minBest,best);}
    const avg=sum/qTokens.length,gram=gramSim(qt,row.title);const exact=row.title===n(qt)||row.sku===n(qt);const score=(exact?220:0)+avg*150+minBest*70+gram*35;
    if(exact||minBest>=minTokenSimilarity||avg>=.78)rows.push({p,score,avg,minBest,gram});
  }
  return rows.sort((a,b)=>b.score-a.score).slice(0,Math.max(1,Math.min(12,Number(limit)||6))).map(x=>({...compactProduct(x.p,{descriptionChars}),score:Number(x.score.toFixed(3)),fuzzy_entity:{average_token_similarity:Number(x.avg.toFixed(3)),minimum_token_similarity:Number(x.minBest.toFixed(3)),gram_similarity:Number(x.gram.toFixed(3))},source:'mig_farm_product_fuzzy_entity_v38'}));
}
