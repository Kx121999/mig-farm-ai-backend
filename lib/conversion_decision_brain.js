...
function evidenceNeeds({message,friction:fr,analysis={},humanTurn={},readiness:r}={}){
  ...
  if(["visual_analysis","visual_followup"].includes(humanTurn?.mode)) needs.push("visual_grounding");
...
function chooseAction({message,friction:fr,readiness:r,humanTurn={},salesTurn={},analysis={}}={}){
  ...
  if(["visual_analysis","visual_followup"].includes(mode)) return "inspect_and_ground_visual_evidence_before_any_sales_action";
...
function questionPolicy({message,readiness:r,humanTurn={},friction:fr,salesTurn={},analysis={}}={}){
  const t=n(message);
  if(["visual_analysis","visual_followup"].includes(humanTurn?.mode)) return {budget:1,ask_for:["single_best_visual_clarification_only_if_needed"],reason:"visual_evidence_first"};
