...
export function mergeSessionState(serverSession={},clientState={}){
  ...
  const merged={...a,...b};
  // V25 action state is server-authoritative. A browser may render it, but it can
  // never create/replace the pending action that authorizes an Odoo mutation.
  if(a.autonomous_action) merged.autonomous_action=a.autonomous_action;
  else delete merged.autonomous_action;
  return merged;
}
