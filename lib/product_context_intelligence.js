...
export function productContextIntent(message="",semanticFrame=null){
  const text=clean(semanticFrame?.normalized?.canonical||message,1600);
  const semanticIntents=Array.isArray(semanticFrame?.intents)?semanticFrame.intents.map(x=>String(x?.name||x)):[];
  if(semanticIntents.includes("comparison"))return "comparison";
  if(semanticIntents.includes("dosage"))return "dosage";
  if(semanticIntents.includes("suitability"))return "suitability";
  if(semanticIntents.includes("price")&&semanticIntents.includes("availability"))return "price_and_availability";
  if(semanticIntents.includes("price"))return "price";
  if(semanticIntents.includes("availability"))return "availability";
  if(semanticIntents.includes("product_details"))return "details";
...
export function resolveProductContext({message="",selectedProduct=null,selectedProducts=[],state={},analysis={},semanticFrame=null}={}){
  const intent=productContextIntent(message,semanticFrame);const currentTurn=turnNumber(state?.turn)+1;
  ...
  const semanticNames=Array.isArray(semanticFrame?.intents)?semanticFrame.intents.map(x=>String(x?.name||x)):[];
  const freshSemanticNeed=(intent==="none"||SEARCH_CUE_RX.test(message))&&semanticNames.includes("product_search")&&Boolean(semanticFrame?.entities?.crops?.length||semanticFrame?.entities?.categories?.length)&&["none","in_turn_product_need"].includes(String(semanticFrame?.reference?.kind||"none"));
  if(freshSemanticNeed)return {action:"clear",intent,reason:"semantic_fresh_product_need",confidence:"high",product:null,products:[]};
  ...
  if(intent==="comparison"){
    const semanticProducts=(Array.isArray(semanticFrame?.reference?.products)?semanticFrame.reference.products:[]).map(normalizeProductReference).filter(Boolean).slice(0,4);
    if(semanticProducts.length>=2)return {action:"compare",intent,reason:"semantic_resolved_product_set",confidence:"high",products:semanticProducts};
  ...
  const semanticProduct=normalizeProductReference(semanticFrame?.reference?.product);
  if(semanticProduct&&intent!=="none")return {action:"bind",intent,reason:"semantic_resolved_reference",confidence:semanticFrame?.reference?.confidence||"medium",product:semanticProduct,products:[]};
