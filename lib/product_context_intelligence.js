...
export function resolveProductContext({message="",selectedProduct=null,selectedProducts=[],state={},analysis={},semanticFrame=null}={}){
  ...
  const freshSemanticNeed=semanticNames.includes("product_search")&&Boolean(semanticFrame?.entities?.crops?.length||semanticFrame?.entities?.categories?.length)&&["none","in_turn_product_need"].includes(String(semanticFrame?.reference?.kind||"none"));
