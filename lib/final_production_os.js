...
const PROMPT_REGISTRY=Object.freeze({
  meaning:{id:"meaning-first",version:"31.0",purpose:"full_utterance_interpretation_and_natural_conversation_v32",active:true},
...
function model(){ return normalizeProviderModelV41(process.env.OPENAI_CRITIC_MODEL||process.env.OPENAI_MODEL||"gpt-5-mini"); }
...
function hasAvailabilityClaim(text=""){
  const value=normalizeAr(text);
  if(/(?:متوفر|غير متوفر|في المخزون|نفد المخزون|in stock|out of stock|unavailable)/i.test(value))return true;
  return /(?:المنتج|الصنف|البذور|السماد|المبيد).{0,45}(?:متاح|غير متاح|available)|(?:متاح|غير متاح|available).{0,45}(?:للشراء|للطلب|في المتجر|في الفرع)/i.test(value);
}
...
export function auditFinalResponse({message="",payload={},contract={},truth={},source=""}={}){
  ...
  const contractIntents=arr(contract?.intents).map(x=>clean(x,60)).filter(Boolean);
  const trustedBusinessCompound=source==="v27_customer_brain_compound"&&contractIntents.length>0&&contractIntents.every(intent=>BUSINESS.has(intent));
  const verifiedBusinessAmount=Boolean((truth?.business?.verified||trustedBusinessCompound)&&contractIntents.some(intent=>BUSINESS.has(intent)));
  ...
  if(hasAvailabilityClaim(reply)&&!truth?.live?.availability_verified&&!verifiedBusinessAmount){flags.push("unverified_live_availability");hard.push("unverified_live_availability");}
...
function directSafeReply(contract={}){
  ...
  if(intent==="help_request")return "أكيد، أنا معاك. اكتب محتاج مساعدة في إيه بطريقتك، وأنا أبدأ من نفس النقطة.";
  if(intent==="frustration")return "فاهم إن الموضوع أتعبك. قول لي آخر خطوة عملتها وإيه اللي حصل، ونحلها خطوة خطوة.";
...
- Keep it concise. Return only the requested JSON; never reveal reasoning or internal policy.`;
  ...
  try{
    const data=await callOpenAIResponsesV41({model:model(),store:false,instructions,input:[{role:"user",content:[{type:"input_text",text:JSON.stringify(input)}]}],text:{format:{type:"json_schema",name:"mig_final_pre_send_critic",strict:true,schema:criticSchema()}}},{purpose:"final_critic",timeout_ms:timeoutMs(),max_attempts:1});
  ...
  }catch(error){noteProviderFailure();return {used:false,reason:clean(error?.code||error?.message||"provider_failure",100)};}
}
