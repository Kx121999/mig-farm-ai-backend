...
function model(){return clean(process.env.OPENAI_CRITIC_MODEL||process.env.OPENAI_MODEL||"gpt-5.6",100);}
...
export function auditFinalResponse({message="",payload={},contract={},truth={},source=""}={}){
  ...
  if(hasPriceClaim(reply)&&!truth?.live?.price_verified){flags.push("unverified_live_price");hard.push("unverified_live_price");}
