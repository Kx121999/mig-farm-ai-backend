...
export function getStructuredProductFacts(identifier=""){
  ...
  return {
    ...
    policy:"Explicit facts are extracted only from stored product text. Missing specifications must remain unknown. Current price/availability require live Odoo."
  };
}
...
export function buildProductTruth(identifier="",liveProducts=[]){
  ...
  return {
    ...
    dossier:{category:dossier.category,supplier:dossier.supplier,type:dossier.type,feature:dossier.feature,description_provenance:dossier.description_provenance,explicit_facts:dossier.explicit_facts,sales_description:dossier.sales_description,weight_kg:num(archived.weight_kg)},
...
export function productTruthHealth(){
  ...
  return {version:VERSION,mode:"live_product_truth_and_sales_action_os",products:Number(FACTS?.stats?.products||0),explicit_facts:Number(FACTS?.stats?.explicit_facts||0),need_terms:Number(FACTS?.stats?.need_terms||0),graph_nodes:Number(GRAPH?.stats?.nodes||0),graph_edges:Number(GRAPH?.stats?.edges||0),alternative_edges:Number(GRAPH?.stats?.alternative_edges||0),shopping_adjacent_edges:Number(GRAPH?.stats?.shopping_adjacent_edges||0),fact_index_bytes:factBytes,relationship_graph_bytes:graphBytes,total_megabytes:Number(((factBytes+graphBytes)/1024/1024).toFixed(2)),current_truth_precedence:"live Odoo price/availability > archived snapshot",compatibility_policy:"relationship graph never proves compatibility",quote_policy:"draft only; live-verified prices required for totals"};
}
