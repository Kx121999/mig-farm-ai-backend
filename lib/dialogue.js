...
export function sanitizeState(value){
  ...
  const activeVisualContext=av.active?{
    ...
    image_count:Math.max(0,Math.min(4,Number(av.image_count)||0)),started_turn:Math.max(0,Number(av.started_turn)||0),last_turn:Math.max(0,Number(av.last_turn)||0),expires_turn:Math.max(0,Number(av.expires_turn)||0),
    ...
    identity_confidence:String(av.identity_confidence||"").slice(0,20),live_verified:Boolean(av.live_verified),label_claim_verified:Boolean(av.label_claim_verified),
  ...
  return {
    v:Number.isFinite(stateVersion)&&stateVersion>=22?stateVersion:Number(v.v)>=12?12:Number(v.v)>=11?11:10,
