...
import { parseAgriculturalProblemV31 } from "./llm_first_orchestrator_v31.js";
...
function asksPlantProblem(message,state={}){ return parseAgriculturalProblemV31(message,state).is_problem; }
...
export function sanitizeState(value){
  ...
  const dc=v.diagnostic_context_v31&&typeof v.diagnostic_context_v31==="object"&&!Array.isArray(v.diagnostic_context_v31)?v.diagnostic_context_v31:{};
  const diagnosticContextV31={
    active:Boolean(dc.active),crop_label:String(dc.crop_label||"").slice(0,80),crop_key:String(dc.crop_key||"").slice(0,80),
    symptoms:Array.isArray(dc.symptoms)?[...new Set(dc.symptoms.slice(0,12).map(x=>String(x||"").slice(0,80)).filter(Boolean))]:[],
    zone:["new_leaves","old_leaves","whole_plant","roots","fruit","flowers","stem","unknown"].includes(String(dc.zone||""))?String(dc.zone):"unknown",
    spread:["localized","spreading","general","unknown"].includes(String(dc.spread||""))?String(dc.spread):"unknown",
    onset:["sudden","gradual","unknown"].includes(String(dc.onset||""))?String(dc.onset):"unknown",
    cultivation:String(dc.cultivation||"").slice(0,60),last_question:String(dc.last_question||"").slice(0,240),expected_field:String(dc.expected_field||"").slice(0,60),
    updated_turn:Math.max(0,Number(dc.updated_turn)||0),expires_turn:Math.max(0,Number(dc.expires_turn)||0)
  };
  const ui=v.intelligence_v33&&typeof v.intelligence_v33==="object"&&!Array.isArray(v.intelligence_v33)?v.intelligence_v33:null;
  const unifiedProduct=item=>({entity_id:String(item?.entity_id||"").slice(0,100),name:String(item?.name||"").slice(0,300),sku:String(item?.sku||"").slice(0,120),external_id:String(item?.external_id||"").slice(0,180),category:String(item?.category||"").slice(0,180),source:String(item?.source||"").slice(0,80),confidence:Math.max(0,Math.min(1,Number(item?.confidence)||0))});
  const intelligenceV33=ui?{
    version:String(ui.version||"33.0").slice(0,20),schema_version:1,turn:Math.max(0,Number(ui.turn)||0),
    active_topic:String(ui.active_topic||"").slice(0,60)||null,active_subtopic:String(ui.active_subtopic||"").slice(0,60)||null,
    active_products:Array.isArray(ui.active_products)?ui.active_products.slice(0,6).map(unifiedProduct).filter(x=>x.name||x.sku||x.external_id):[],
    visible_products:Array.isArray(ui.visible_products)?ui.visible_products.slice(0,6).map(unifiedProduct).filter(x=>x.name||x.sku||x.external_id):[],
    active_product_id:String(ui.active_product_id||"").slice(0,100)||null,active_crop:String(ui.active_crop||"").slice(0,100)||null,
    active_pest:String(ui.active_pest||"").slice(0,100)||null,active_disease:String(ui.active_disease||"").slice(0,100)||null,
    active_problem:ui.active_problem&&typeof ui.active_problem==="object"?{kind:String(ui.active_problem.kind||"").slice(0,60),description:String(ui.active_problem.description||"").slice(0,500),symptoms:Array.isArray(ui.active_problem.symptoms)?ui.active_problem.symptoms.slice(0,10).map(x=>String(x).slice(0,100)):[],source:String(ui.active_problem.source||"").slice(0,60),turn:Math.max(0,Number(ui.active_problem.turn)||0)}:null,
    active_environment:String(ui.active_environment||"").slice(0,100)||null,active_location:String(ui.active_location||"").slice(0,100)||null,
    active_quantity:ui.active_quantity!==null&&ui.active_quantity!==""&&Number.isFinite(Number(ui.active_quantity))?Number(ui.active_quantity):null,
    user_goal:String(ui.user_goal||"").slice(0,300)||null,known_constraints:Array.isArray(ui.known_constraints)?ui.known_constraints.slice(0,16).map(x=>String(x).slice(0,180)):[],
    confirmed_facts:Array.isArray(ui.confirmed_facts)?ui.confirmed_facts.slice(-24).map(x=>({key:String(x?.key||"").slice(0,80),value:String(x?.value||"").slice(0,320),source:String(x?.source||"").slice(0,80),turn:Math.max(0,Number(x?.turn)||0),confidence:Math.max(0,Math.min(1,Number(x?.confidence)||0))})).filter(x=>x.key&&x.value):[],
    uncertain_facts:Array.isArray(ui.uncertain_facts)?ui.uncertain_facts.slice(-16).map(x=>String(x).slice(0,320)):[],pending_question:String(ui.pending_question||"").slice(0,420)||null,
    last_assistant_action:String(ui.last_assistant_action||"").slice(0,100)||null,last_reference_resolution:ui.last_reference_resolution&&typeof ui.last_reference_resolution==="object"?{kind:String(ui.last_reference_resolution.kind||"").slice(0,60),resolved:Boolean(ui.last_reference_resolution.resolved),entity_id:String(ui.last_reference_resolution.entity_id||"").slice(0,100)||null,entity_name:String(ui.last_reference_resolution.entity_name||"").slice(0,300)||null,confidence:Math.max(0,Math.min(1,Number(ui.last_reference_resolution.confidence)||0))}:null,
    last_route:String(ui.last_route||"").slice(0,60)||null,last_rewritten_query:String(ui.last_rewritten_query||"").slice(0,1800)||null,last_correction:ui.last_correction&&typeof ui.last_correction==="object"?ui.last_correction:null,updated_at:String(ui.updated_at||"").slice(0,40)
  }:null;
  ...
  return {
    ...
    diagnostic_context_v31:diagnosticContextV31,
    customer_digital_twin_v30:customerDigitalTwinV30,
    ...(intelligenceV33?{intelligence_v33:intelligenceV33}:{}),
...
export function analyzeTurn(message,state={},history=[],locale="ar"){
  ...
  let intent="unknown";
  if(naturalConversation) intent=naturalConversation.intent;
  else if(onlyGreeting(t)) intent="greeting";
  ...
  else if(asksPlantProblem(message,state)) intent="plant_problem";
  ...
  return {intent,t,category,crop,knownProduct,seedVarieties,emirate,cultivation,quantity,budget,pepperType,memoryAction:mem,faq:FAQMatch(message),locale,correction,correctionText,hardSwitch:hardSwitchCue(message),naturalConversation};
}
...
export function updateState(previous,analysis,message,source="",results=[],responsePayload={}){
  ...
  const diagnosisTurn=analysis.intent==="plant_problem"||analysis.intent==="diagnosis"||analysis.v31_primary_intent==="diagnosis";
  if(diagnosisTurn){
    const parsed=parseAgriculturalProblemV31(message,s),old=s.diagnostic_context_v31||{},sameTopic=Boolean(parsed.followup&&old.active);
    const cropLabel=String(analysis.crop?.labelAr||parsed.crop||(sameTopic?old.crop_label:"")||"").slice(0,80);
    const baseSymptoms=sameTopic&&Array.isArray(old.symptoms)?old.symptoms:[];
    const symptoms=[...new Set([...baseSymptoms,...(Array.isArray(analysis.symptoms)?analysis.symptoms:[]),...parsed.symptoms].map(x=>String(x||"").slice(0,80)).filter(Boolean))].slice(0,12);
    const nextTurn=(s.turn||0)+1;
    s.diagnostic_context_v31={
      active:true,crop_label:cropLabel,crop_key:String(analysis.crop?.key||(sameTopic?old.crop_key:"")||"").slice(0,80),symptoms,
      zone:parsed.zone!=="unknown"?parsed.zone:(sameTopic?old.zone:"unknown"),spread:parsed.spread!=="unknown"?parsed.spread:(sameTopic?old.spread:"unknown"),onset:parsed.onset!=="unknown"?parsed.onset:(sameTopic?old.onset:"unknown"),
      cultivation:String(analysis.cultivation||s.cultivation||(sameTopic?old.cultivation:"")||"").slice(0,60),
      last_question:String(analysis.diagnosticQuestion||parsed.question||"").slice(0,240),expected_field:String(analysis.diagnosticExpectedField||"").slice(0,60),
      updated_turn:nextTurn,expires_turn:nextTurn+6
    };
  }else if(["greeting","wellbeing","thanks","goodbye","identity","help_request","frustration","general_conversation","branches","shipping","delivery_time","contact","hours","payment","returns","order_status","complaint"].includes(analysis.intent)){
    s.diagnostic_context_v31={...(s.diagnostic_context_v31||{}),active:false};
  }
  s.turn=(s.turn||0)+1;
  return s;
}
...
function plantProblemContext(analysis={},state={},message=""){
  const parsed=parseAgriculturalProblemV31(message||analysis.t||"",state);
  const symptoms=[...new Set([...(Array.isArray(analysis.symptoms)?analysis.symptoms:[]),...parsed.symptoms])];
  return {...parsed,crop:analysis.crop?.labelAr||parsed.crop||"",symptoms,stateCrop:state.crop||""};
}
...
export function directReply(analysis,state,message,sessionId=""){
  ...
  if(["help_request","frustration","general_conversation"].includes(analysis.intent)){
    const frame=analysis.naturalConversation||classifyNaturalConversationV32(message)||{intent:analysis.intent,subtype:"help",female:false};
    return {reply:composeNaturalConversationReplyV32(frame,locale),source:`natural_${analysis.intent}_v32`};
  }
  ...
  if(analysis.intent==="plant_problem"){
    const context=plantProblemContext(analysis,state,message);
    if(context.crop&&context.symptoms.length){
      const symptomText=context.symptoms.join(" و");
      return {reply:`فهمت: عندك ${context.crop} وفيه ${symptomText}. ما ينفعش نثبت مرض أو نختار مبيد من الوصف وحده؛ هنفرّق بين الأسباب حسب الجزء المصاب، الانتشار، وقت البداية، والري. ${context.question} ولو تقدر أرسل صورة واضحة للجزء المصاب وصورة كاملة للنبات.`,source:"plant_diagnosis_contextual",actions:[{type:"whatsapp",label:"أرسل صورة الإصابة",url:BUSINESS.whatsapp}]};
    }
...
export function ambiguousContextReply(message,state,analysis){
  ...
  if(words(t).length<=3){
    const quoted=String(message||"").trim().slice(0,80);
    return {reply:quoted?`عايز أتأكد إني فاهمك: تقصد إيه بـ «${quoted}»؟ اكتبها بطريقتك حتى لو بجملة قصيرة.`:"اكتب لي اللي محتاجه بطريقتك، وأنا أبدأ معاك من نفس الكلام.",source:"clarify_current_words_v32"};
  }
  return null;
}
