...
const SOCIAL=/^(اهلا|أهلا|هلا|السلام عليكم|سلام|هاي|hello|hi|hey|صباح الخير|مساء الخير|عامل ايه|عامل إيه|اخبارك|أخبارك|شلونك|كيفك|شو اخبارك|وش اخبارك|ازيك|إزيك|شكرا|شكراً|تسلم|تمام|اوكي|أوكي|ماشي|مين انت|مين انتا|من انت|انتا مين|انت مين|أنت مين|اسمك ايه|اسمك إيه|شو اسمك|وش اسمك|منو انت|هههه+|هاها+|😂+|🤣+)[\s.!؟?]*$/;
...
function mode(message,analysis={},visionContext={}){
  ...
  if(SOCIAL.test(t)||["greeting","wellbeing","thanks","goodbye","acknowledgment","negative_ack","identity","human"].includes(intent)) return "social";
...
export function safeCurrentTurnFallback(message="",humanTurn={}){
  ...
  if(m==="social"){
    const intent=String(humanTurn?.current_topic?.intent||"");
    if(intent==="identity"||/(?:مين|من|منو)\s+(?:انت|انتا)|(?:انت|انتا)\s+مين|اسمك\s+(?:ايه|إيه|شو|وش)/.test(t)) return "أنا MIG FARM AI، مساعد ذكي لـ MIG FARM للمنتجات والزراعة وخدمات المتجر. مش موظف بشري، لكن أقدر أفهم سؤالك وأساعدك مباشرة.";
    if(/عامل ايه|عامل إيه|اخبارك|أخبارك|كيفك|شلونك|شو اخبارك|وش اخبارك|ازيك|إزيك/.test(t)||intent==="wellbeing") return "تمام الحمد لله 😄 إنت عامل إيه؟";
    if(/شكرا|شكراً|تسلم/.test(t)||intent==="thanks") return "حبيبي، تحت أمرك.";
    if(/هههه|😂|🤣/.test(String(message))) return "😂😂 تمام وصلت.";
    if(intent==="goodbye") return "في أمان الله 👋";
    return "أهلًا بيك 👋";
  }
