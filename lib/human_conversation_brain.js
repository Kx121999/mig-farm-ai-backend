...
function mode(message,analysis={}){
...
function responseLength(message,m){const wc=words(message).length;if(["social","browse_only_social"].includes(m))return "micro";if(wc<=6)return "short";if(wc<=22)return "compact";if(m==="technical"||m==="legal")return "adaptive_detailed";return "normal"}
function toolPolicy(m,followup){
  ...
  if(m==="followup") return {mode:"followup",allowed:["search_catalog","search_knowledge","recall_memory","compare_live_options","search_agricultural_engineering","search_agricultural_master","diagnose_crop_problem","get_business_fact"]};
  if(m==="repair_or_switch") return {mode:"current_turn_only",allowed:["search_catalog","search_knowledge","search_agricultural_engineering","diagnose_crop_problem","search_agricultural_master","search_uae_agriculture","search_uae_regulations","get_business_fact"]};
  if(m==="legal") return {mode:"legal_evidence",allowed:["search_uae_regulations","search_uae_agriculture","search_agricultural_master"]};
  if(m==="technical") return {mode:"agricultural_expert",allowed:["search_agricultural_engineering","search_agricultural_master","diagnose_crop_problem","agriculture_calculator","search_uae_agriculture","search_uae_regulations","search_catalog"]};
  if(m==="purchase"||m==="commerce") return {mode:"commerce",allowed:["search_catalog","search_knowledge","get_business_fact","search_sales_playbook","optimize_live_bundle","compare_live_options","prepare_purchase_plan","search_agricultural_engineering","search_agricultural_master","diagnose_crop_problem"]};
...
export function analyzeHumanConversationTurn(message="",{analysis={},history=[],state={},profile={},agriculturalContext={}}={}){
  const t=n(message); const baseMode=mode(message,analysis); const followup=isFollowup(message,analysis); const m=(baseMode==="general_conversation"&&followup)?"followup":baseMode; const cp=contextPolicy(m,followup,message); const tp=toolPolicy(m,followup);
  ...
  return {
    ...
    response_contract:{answer_first:true,length:responseLength(message,m),question_budget:["social","browse_only_social"].includes(m)?0:1,mirror_dialect:true,mirror_energy:true,no_canned_intro:true,no_unasked_agronomy:["social","browse_only_social"].includes(m),no_forced_sale:Boolean(noSales),one_natural_idea_per_short_turn:true},
    stale_context_quarantine:!cp.allow_stale_product_context,
    explicit_repair:m==="repair_or_switch",
    debug_reason:m==="browse_only_social"?"user_not_buying_or_browsing":m==="repair_or_switch"?"user_repair_or_topic_switch":followup?"explicit_followup":"current_turn_semantics"
  };
}
