...
export function buildConversationStateV33({message="",state={},meaningFrame={},selectedProduct=null,selectedProducts=[],visionFrame=null}={}){
  ...
  if(reference.resolved&&reference.product){
    const product=reference.product;const remaining=correction?[]:arr(next.active_products).map(x=>normalizeProduct(x,"active_state")).filter(Boolean).filter(x=>x.entity_id!==product.entity_id);
...
function degradedReply({message="",meaningFrame={},conversationState={},route={}}={}){
  ...
  const active=arr(conversationState.active_products).find(x=>x.entity_id===conversationState.active_product_id)||arr(conversationState.active_products)[0];
  const effectiveIntent=route?.corrected_goal_intent||route?.primary_intent||meaningFrame?.primary_intent||"unknown";
  // Provider failure must not turn a simple human/social turn into a system-error message.
  // These are semantic-intent fallbacks, not phrase matching and never replace the primary LLM path.
  if(effectiveIntent==="identity")return "أنا MIG FARM AI، مساعد ذكي لـ MIG FARM للمنتجات والزراعة وخدمات المتجر. مش موظف بشري، لكن أقدر أساعدك مباشرة.";
  if(effectiveIntent==="wellbeing")return "تمام الحمد لله 😄 إنت عامل إيه؟";
  if(effectiveIntent==="greeting")return "أهلًا بيك 👋";
  if(effectiveIntent==="thanks")return "العفو، تحت أمرك 🌱";
  if(effectiveIntent==="goodbye")return "في أمان الله 👋";
  if(["acknowledgment","negative_ack"].includes(effectiveIntent))return effectiveIntent==="negative_ack"?"تمام، فهمت إن ده مش اللي تقصده.":"تمام 👍";
  if(effectiveIntent==="help_request")return "أكيد، قول لي محتاج مساعدة في إيه بطريقتك وأنا أبدأ من نفس النقطة.";
  if(effectiveIntent==="human")return "أقدر أساعدك هنا، ولو محتاج موظف بشري قل لي وهنمشي لخطوة التواصل المناسبة.";
  ...
  return "وصلني سؤالك، لكن مولّد الإجابة الذكي تعذر عليه الرد في المحاولة دي. جرّب مرة ثانية بعد لحظة.";
}
