...
export function buildConversationStateV33({message="",state={},meaningFrame={},selectedProduct=null,selectedProducts=[],visionFrame=null}={}){
  ...
  next.active_topic=domain!=="unclear"?domain:(next.active_topic||"unclear");next.active_subtopic=primary!=="unknown"?primary:next.active_subtopic;
  ...
  if(correction)next.last_correction={turn,replacement:{product:reference.product?.name||null,crop:entities.crop||null,topic:domain},supersedes_previous:true};
  return next;
}
...
export function routeIntelligenceV33({meaningFrame={},conversationState={},hasImages=false}={}){
  const intents=intentNames(meaningFrame),domain=routeDomain(meaningFrame),primary=intents[0]||"unknown";
  let kind="conversation_only";
  if(hasImages||primary==="image_analysis")kind="multimodal";
  else if(intents.every(x=>SOCIAL.has(x)))kind="conversation_only";
  else if(intents.some(x=>AGRICULTURE.has(x)))kind="technical";
  else if(intents.some(x=>["purchase","bundle","compare"].includes(x)))kind="commerce";
  else if(intents.some(x=>EXACT_PRODUCT.has(x)))kind="product_exact";
  else if(intents.some(x=>PRODUCT.has(x)))kind="product_discovery";
  else if(intents.some(x=>BUSINESS.has(x)))kind="business";
  ...
  if(primary==="correction"&&hasProductContext&&["product_exact","product_discovery","commerce"].includes(clean(conversationState?.last_route,40)))kind="product_discovery";
  if(domain==="mixed"||intents.some(x=>BUSINESS.has(x))&&intents.some(x=>PRODUCT.has(x)||AGRICULTURE.has(x)))kind="multi_source";
  ...
  const tasks=intents.filter(x=>x!=="unknown").map((intent,index)=>({id:`task_${index+1}`,intent,source:BUSINESS.has(intent)?"business_fact":AGRICULTURE.has(intent)?"technical_knowledge":["price","availability"].includes(intent)?"live_structured_data":PRODUCT.has(intent)?"product_data":"conversation",status:"pending"}));
  return {version:VERSION,kind,domain,primary_intent:primary,intents,tasks,allowed_tools:allowed,requires_structured_data:intents.some(x=>["price","availability","purchase","order_status"].includes(x)),requires_semantic_knowledge:intents.some(x=>["diagnosis","agriculture_general","suitability","recommendation","compare","product_search"].includes(x)),requires_generation:true,max_retrieval_attempts:2,active_entity_id:conversationState.active_product_id||null};
}
...
export function rewriteQueryV33({message="",meaningFrame={},conversationState={},route={}}={}){
  ...
  if(route?.intents?.length)parts.push(`المطلوب: ${route.intents.join("، ")}`);
...
function degradedReply({message="",meaningFrame={},conversationState={},route={}}={}){
  ...
  const active=arr(conversationState.active_products).find(x=>x.entity_id===conversationState.active_product_id)||arr(conversationState.active_products)[0];
  if(["price","availability","product_details","known_product_info","dosage"].includes(route?.primary_intent)&&active?.name)return `فهمت إنك بتسأل عن ${active.name}، لكن ما قدرتش أوصل للمصدر المطلوب دلوقتي من غير ما أخمّن. جرّب إعادة المحاولة بعد لحظة.`;
