...
function compactProduct(p={}){
  return {
    ...
    truth:p.truth&&typeof p.truth==="object"?{
      source:String(p.truth.source||"").slice(0,80),
      observed_at:String(p.truth.observed_at||"").slice(0,40),
      ttl_seconds:Math.max(60,Math.min(3600,Number(p.truth.ttl_seconds)||600)),
      current:Boolean(p.truth.current)
    }:undefined
  };
}
