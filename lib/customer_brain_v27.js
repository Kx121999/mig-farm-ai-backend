...
function extractEntities(text){
  ...
  const productReference=clean(text.replace(/[؟?!.،]/g," ").replace(/(?:مكانكم|موقعكم|فين|وين|هل|هو|السعر|بكام|بكم|متوفر|موجود|عايز|أبغي|ابي|أبي|محتاج|تفاصيل|توصيل|شحن|وال|و)/gi," "),260);
  return {emirate,crop,category,quantity,budget,sku,product_reference:productReference};
}
...
export function buildCustomerBrainFrameV27({message="",analysis={},semanticFrame=null,state={}}={}){
  ...
  const entities=extractEntities(text);const multi=tasks.length>1||Boolean(semanticFrame?.compound?.is_multi_intent);
