...
for(const file of files){
  ...
  const v41=file.startsWith("v41_");
  const v40=file.startsWith("v40_");
  const v33=file.startsWith("v33_");
  const result=spawnSync(process.execPath,[path],{cwd:root.pathname,stdio:"inherit",env:{...process.env,AI_PIPELINE_V41:v41?"true":"false",AI_PIPELINE_V40:(v41||v40)?"true":"false",AI_PIPELINE_V33:(v41||v40||v33)?"true":"false"}});
  if(result.status!==0){
    console.error(`\nFAILED: ${file}`);
