...
export async function GET(request){
  ...
  return new Response(JSON.stringify({exported_at:new Date().toISOString(),version:"31.0.0",privacy_safe:true,telemetry:snapshot},null,2),{
    status:200,headers:adminSecurityHeadersV28({"Content-Type":"application/json; charset=utf-8","Content-Disposition":`attachment; filename="mig-farm-v31-report-${new Date().toISOString().slice(0,10)}.json"`})
  });
}
