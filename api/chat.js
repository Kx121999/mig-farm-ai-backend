...
import { naturalIntentReply, smallTalk, formatProducts, extractPageAnswer } from "../lib/emirati.js";
...
function isProductFollowup(message){
  const t=normalizeAr(message);
  return /(طب|طيب|زين|والحار|والحلو|ارخص|الارخص|اغلى|الاغلى|سعره|سعرها|بكم|بكام|تفاصيله|تفاصيلها|متوفر|موجود|فيهم|منهم|هذا|هذي|ده|دي)/.test(t);
}
...
export async function POST(request){
  ...
  if(!rateLimit(`${ip}:${sessionId}`)){
    return jsonResponse({
      ...
      suggested_actions:[],
      escalation:false,
      mode:"free_sitewide_emirati_v3_1"
    },429,cors);
  }
  ...
  const natural=naturalIntentReply(message,locale);
  if(natural){
    return jsonResponse({
      reply:natural,
      session_id:sessionId,
  ...
  try{
    const hasRecentProductContext =
      isProductFollowup(message) &&
      history.slice(-4).some(x=>x.role==="user" && asksProductish(x.content));

    if(asksProductish(message) || hasRecentProductContext){
